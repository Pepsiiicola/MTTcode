function [fusion_x,fusion_w,fusion_P] = ALG1_Matching(s1_w_bar_k,s1_m_bar_k,s1_P_bar_k,s2_w_bar_k,s2_m_bar_k,s2_P_bar_k)
%**********************************************************************
%                     传感器CI匹配融合
...
for fusion_k=1:s1_L
    ...
    if (isempty(dist)) %判断dist向量是否为空，为空就意味着传感器2中的高斯分量已被匹配完
        ...
        if min_dist<fusion_gate %若最小距离小于融合阈值则进行CI融合
            [fusion_x_k,fusion_w_k,fusion_P_k] = ALG1_AA_fusion(s1_w_bar_k(fusion_k),s1_m_bar_k(:,fusion_k),s1_P_bar_k(:,4*fusion_k-3:4*fusion_k),s2_w_bar_k(min_c),s2_m_bar_k(:,min_c),s2_P_bar_k(:,4*min_c-3:4*min_c));
            %删除融合过的分量
            s2_w_bar_k(min_c)=[];
            s2_m_bar_k(:,min_c)=[];
