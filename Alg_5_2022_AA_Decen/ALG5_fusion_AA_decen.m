%{
                           分散式AA融合方法
%}
function [Results] = ALG5_fusion_AA_decen(Fusion_center,match_threshold,mat_weight,ID_sensor,l,mat_topo)
...
for i = 1:N_sensor
    InfComplete.w = [InfComplete.w [Fusion_center.Inf_recieve(i).gm_particles{1,1}(:,1:end)]];
    InfComplete.m = [InfComplete.m [Fusion_center.Inf_recieve(i).gm_particles{2,1}(:,1:end)]];
    InfComplete.P = [InfComplete.P [Fusion_center.Inf_recieve(i).gm_particles{3,1}(:,1:end)]];
...
if ID_record ~= 0
    ...
    for i = ID_record+1 : N_sensor  %从ID_record+1开始遍历，不包括自己
        ...
        %===匹配部分===
        [Solution_match,Mat_match] = ALG5_Match(Match_part1,Match_part2,Matched_GM,cnt_matchedGm,match_threshold);
...
if ID_record == 0
    ...
    for i = 1:n_fusion
        ...
        M_AA = [0 0 0 0 0 0]';
        for j = 1:n_w2f
            M_AA = M_AA + pai_state(j) * Matched_GM(i).gm{2,1}(1:6,j);
        end
        
        % 协方差融合
        P_AA = zeros(6,6);
        for j = 1:n_w2f
            P_AA = P_AA +pai_state(j) * ( Matched_GM(i).gm{3,1}(:,6*(j-1)+1:6*j) +...
                ( M_AA - Matched_GM(i).gm{2,1}(1:6,j) )*( M_AA - Matched_GM(i).gm{2,1}(1:6,j))' );
        end
...
%先进行粒子所属节点视域的判断
        ...
        if ~isempty(noprovide_sensorID)   
            ...
            if ~isempty(intersect(noprovide_sensorID,noprovide_ok_sensorID))
                ...
                %判断除了不需要提供粒子的邻居节点，还是否存在该提供却没提供粒子的邻居节点
...
%         W_AA = W_AA * min( 1, n_w2f / cnt_fovIn); % 对权重比调整过高的情况进行一致抑制
