...
%}
function [OSPA,Num_estimate,Time] = PROCESS_OF_ALG6(~,Xreal_time_target,Sensor,N,L,mat_weight)
...
for t=3:N
    ...
    %==================
    for i=1:N_sensor 
        [Sensor(i).state_ex]=ALG6_PHD1time_3d_ukf_decen(Sensor(i).Z_dicaer_global{t-2,1},...
    ...
    for l = 1:L
        ...
        % 分散式GA融合方式
        for i=1:N_sensor
            [Sensor(i).state_ex] = ALG6_fusion_GA_decen(Fusion_center(i),match_threshold,mat_weight,...
