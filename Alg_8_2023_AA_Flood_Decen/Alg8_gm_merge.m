function [merge_result] = Alg8_gm_merge(state,U)
...
while ~isempty(I)%合并时会从I中删除
    ...
    for i = L
%         m_kb(:,l) = m_kb(:,l) + w_k(1,i)*m_k(:,i);
        m_kb(:,l) = m_kb(:,l) + w_k(1,i)/w_kb(1,l)*m_k(:,i);
    end
%     m_kb(:,l) = (1/w_kb(1,l))*m_kb(:,l);
    for i = L
%         P_kb(:,Index(l)) = P_kb(:,Index(l)) + w_k(1,i)*(P_k(:,Index(i))+(m_kb(:,l)-m_k(:,i))*(m_kb(:,l)-m_k(:,i))');
        P_kb(:,Index(l)) = P_kb(:,Index(l)) + w_k(1,i)/w_kb(1,l)*(P_k(:,Index(i))+(m_kb(:,l)-m_k(:,i))*(m_kb(:,l)-m_k(:,i))');
    end
%     P_kb(:,Index(l)) = (1/w_kb(1,l))*P_kb(:,Index(l));
