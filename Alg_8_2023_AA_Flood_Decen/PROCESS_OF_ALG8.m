%{
           ...
           注： 以1号传感器的结果作为结果输出
%}
function [OSPA,Num_estimate,Time] = PROCESS_OF_ALG8(Xreal_target_time,Xreal_time_target,Sensor,N,L)
...
%             Input(j).w_bar_k,Input(j).m_bar_k,Input(j).Z{k,1},Input(j).Z_clutter,Input(j).nClutter,Input(j).detect_prob,Input(j).x_k_history,k,Input(j).location,Input(j).R_detect);
            [Input(j).x_k_history,Input(j).w_birth,Input(j).m_birth,Input(j).P_birth,Input(j).numTargetbirth,Input(j).w_bar_k, Input(j).m_bar_k,Input(j).P_bar_k,Output(j).x_k,Output(j).x_k_w,Output(j).x_k_P] = Alg8_GM_PHD_Filter(Input(j).numTargetbirth,Input(j).m_birth,Input(j).P_birth,Input(j).w_birth,F,Q,R,...
       ...
       for i=1:sensor_Num
        ...
        for l=1:L
            %===========信息发送，发送到公共平台，等待接收=============
%             for i=1:sensor_Num
%                 com_storage(i).gm_components = storage_Inf(i).temp;
%             end

            %========信息接收,注意：包括节点自身的信息===========
%             for i=1:sensor_Num
%                 [Fusion_center(i).Inf_recieve] = Alg8_module_com_recieve(com_storage,Sensor(i).mat_topo,i,l); 
%             end

            %=======信息接收，每个节点接收邻居的信息==== 12.21修改
            for i = 1:sensor_Num
                [Fusion_center(i).Inf_recieve,Fusion_center(i).Inf_topo] = Alg8_module_com_recieve_modif(com_storage,Sensor(i).mat_topo,i,l,Fusion_center);
            end

            %======通信带宽受限情况  ==== 3.7增加
%             for i = 1:sensor_Num
%                 [Fusion_center(i).Inf_recieve] = ALG8_BandWidth_limit(Fusion_center(i).Inf_recieve,Sensor(i).mat_topo,Sensor(i).Com_BandWidth,i);
%             end


            %======与之前的信息进行合并========12.21修改
            for i = 1:sensor_Num
                [Fusion_center(i)] = Alg8_Inf_merge(Fusion_center(i));
            end
...
%             end
            ...
            for i = 1:sensor_Num
                [com_storage(i)] = ALG8_Inf_assemble(Fusion_center(i),i);
            end

            %====将每个节点的信息进行去重复处理=========12.26修改
            for i = 1:sensor_Num
                [Fusion_center(i)] = Alg8_Inf_deduped(Fusion_center(i));
            end
        ...
        %==================通信结束，开始融合==============
        for i = 1:sensor_Num
            [Sensor(i).state] = ALG8_fusion_diffusion(Fusion_center(i),match_threshold,...
                i,l,Sensor);
        end
        ...
        %==================状态提取=================
        for i = 1:sensor_Num
            [Sensor(i).X_est{k,1},Sensor(i).num_est(k)] = Alg8_statedraw(Sensor(i).state);
        end
