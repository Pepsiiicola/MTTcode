...
function solution = Murty(costMat, k)   %%%%{assgmt, cost}  assgmt is an nMeas*1 matrix giving the ith minimum assignment 
 
solution = cell(1, k);

t = 1;
[assgmt, cost] = Hungarian_M(costMat);
...
while t < k
    ...
    if idxRec == -1 % all node in the nodeList leads to inf cost
        ...
        nodeList = [nodeList(idx), MurtyPartition(nodeRec, assgmtRec, 1)];%обр╩╫в╤н
    end
end
